...
namespace BMEffects_Remaster
{
    ...
    public class BMEffects_Remaster : GamePlugin_PostLoad
    {
        ...
        public Version version
        {
            get
            {
                return new Version(BMEffects_Remaster.ver);
            }
        }
        ...
        {
            AssetBundleDefinition? bundle = Configured.i.AssetBundles.Find(new Guid(BMEffects_Remaster.guid));

            if (bundle == null!)
            {
                AdvLogger.Log(LogPriority.Error, $"Bundle (guid: {BMEffects_Remaster.guid}) not found!", LogOptions.Popup);
