...
namespace BMEffects_Remaster.Effects.Muzzle
{
    ...
    internal class BM_AdvExtend
    {
        private static bool Prefix(AdvCannonFiringPiece __instance, bool localSource, ref SoundEventRegulator ____firingSoundRegulator)
        {
            try
            {
                ...
                if (num > 0)
                {
                    GameObject gameObject = BM_EffectCreator.Instance.CreateMuzzle(type);
                    if (gameObject != null)
                    {
                ...
                if (flag)
                {
                    ...
                    GameObject gameObject2 = BM_EffectCreator.Instance.CreateMuzzle(type2);
                    if (gameObject2 != null)
                    {
                ...
                BM_EffectCreator.Instance.CreateImpactSplash(vector, radius);
            }
            catch
            {
            }
