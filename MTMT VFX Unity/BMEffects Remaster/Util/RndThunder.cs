...
public class RndThunder : MonoBehaviour
{
	...
	private void Update()
	{
		if (this.time > this.RldTime)
		{
			...
			{
				this.time = 0f;
				GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(this.obj, base.transform.position + Vector3.up * 0.25f, Quaternion.AngleAxis(UnityEngine.Random.Range(90f, 90f), Vector3.left) * Quaternion.AngleAxis(UnityEngine.Random.Range(0f, 360f), Vector3.forward));
