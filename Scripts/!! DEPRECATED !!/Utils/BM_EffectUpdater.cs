...
namespace BMEffects_Remaster.Util
{
    public class BM_EffectUpdater : MonoBehaviour
    {
        public void Start()
        {
            ...
            LightObject = new GameObject("LightObject");
        ...
        public void OnGUI()
        {
            if (gui.bShowFPS)
            {
                GUI.Label(new Rect(10f, 10f, 120f, 40f), "FPS:" + fps);
            }
            GUI.Label(new Rect(10f, 100f, 1024f, 100f), DbgMessage);
        }
        ...
        public void LateUpdate()
        {
            ...
            if (!bInit)
            {
                ...
                foregroundCamera.GetComponent<PostProcessingBehaviour>().profile = (PostProcessingProfile)BMEffects_Remaster.assetBundle.LoadAsset("BM_PPP");
                ...
                gui.UpdateEffects();
                BM_Trail.mat = (Material)BMEffects_Remaster.assetBundle.LoadAsset("FireLoop 1");
            ...
            DirectionalLight.transform.forward = DirectionalSun.SunPosition;
            DirectionalLight.color = DirectionalSun.Light.color;
            DirectionalLight.intensity = DirectionalSun.Light.intensity;
            if (Input.GetKeyDown(122))
            {
                allname = "";
                foreach (GameObject obj2 in Array.FindAll<GameObject>(Object.FindObjectsOfType<GameObject>(), (item) => item.transform.parent == null))
                {
                    AllGameObject(obj2, 0);
                }
                Debug.Log(allname);
            }
        }
        ...
        public void AllGameObject(GameObject obj, int layer)
        {
            ...
            allname = allname + text + obj.name;
            allname += "\n";
            for (int j = 0; j < obj.transform.childCount; j++)
            {
                AllGameObject(obj.transform.GetChild(j).gameObject, layer + 1);
            }
        }
        ...
        public void FindBlocks(GameObject obj, int layer)
        {
            ...
            if (component != null)
            {
                ...
                {
                    allname = allname + text + block.Name;
                    allname += "\n";
                }
                allname += "\n";
            }
            for (int j = 0; j < obj.transform.childCount; j++)
            {
                FindBlocks(obj.transform.GetChild(j).gameObject, layer + 1);
            }
        }
