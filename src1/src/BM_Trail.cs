...
{
	// Token: 0x0600000F RID: 15 RVA: 0x0000256C File Offset: 0x0000076C
	public void Start()
	{
		this.TrailL = null;
		this.TrailR = null;
	...
	public void Init()
	{
		...
		this.TrailL.transform.parent = this.c.GameObject.transform;
		TrailRenderer trailRenderer = this.TrailL.AddComponent<TrailRenderer>();
		this.TrailR = new GameObject("TrailR");
		this.TrailR.transform.parent = this.c.GameObject.transform;
	...
	public void Update()
	{
		...
		{
			this.TrailL.transform.localPosition = this.c.ArrayBasics.AliveAndDead.Blocks[this.nNowTrailL].LocalPosition;
		}
		if (this.nNowTrailR != -1)
		{
			this.TrailR.transform.localPosition = this.c.ArrayBasics.AliveAndDead.Blocks[this.nNowTrailR].LocalPosition;
		}
		...
		if (this.nCounter >= this.c.ArrayBasics.AliveAndDead.Count)
		{
			...
			if (!this.c.ArrayBasics.AliveAndDead.Blocks[this.nNowTrailL].isAlive)
			{
				this.nNowTrailL = -1;
				this.MinX = 65535;
				this.MinXZ = 65535;
			}
			if (!this.c.ArrayBasics.AliveAndDead.Blocks[this.nNowTrailR].isAlive)
			{
				this.nNowTrailR = -1;
				this.MaxX = -65535;
				this.MaxXZ = 65535;
